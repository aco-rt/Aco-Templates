...
1800, //Respawn time.
{(group _this) setVariable ["zhc_offload_blacklisted",true,true]; (group _this) setVariable ["acex_headless_blacklist",true,true];}, //Custom init code for each spawned vehicle.
...
cas_helo_2,
"Talon 6-4 (UH-4 Hellcat)", //Callsign.  Leave blank for default.
1800, //Respawn time.
{(group _this) setVariable ["zhc_offload_blacklisted",true,true]; (group _this) setVariable ["acex_headless_blacklist",true,true];}, //Custom init code for each spawned vehicle.
["FRXA_tf_rt1523g_Ranger_Green","FRXA_tf_rt1523g_big_Ranger_Green","vn_b_pack_prc77_01","vn_b_pack_lw_06"],  //Access items.  Player must have one to use the asset remotely.
"true", //Access condition.  This must be true if the player can use it at all.
