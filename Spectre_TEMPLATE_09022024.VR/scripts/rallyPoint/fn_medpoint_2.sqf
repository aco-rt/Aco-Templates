	...
	if(count _num >1) then
		{
			...
			[20,[],{medpoint_2 setDir ((getDir Player)-180); medpoint_2 setVehiclePosition [(player modelToWorld[0,2,0.1]), [],0.2, "CAN_COLLIDE"]; hint "Medical Station Deployed"},{hint "Deploying interupted"},"Deploying Medical Station"] call ace_common_fnc_progressBar
		} 
	else 
		{
